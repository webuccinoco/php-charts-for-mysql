...
$left_field = (isset($_POST["left_field"])) ? $_POST['left_field'] : '';
	...
	@$tablesFilters = $_POST['tables_filters'];
...
if(!empty($continue))
{
  if(!empty($tablesFilters))
  {

	  $_SESSION["ks_tables_filters"]=$tablesFilters;
...
function print_tables_filters($field_name)
{
	...
	if(isset($_POST[$field_name]) && $_POST[$field_name] == "Between" && $type != 0)
	{
...
{
	global $tablesFilters;

	if(!isset($_POST['left_table']) && isset($_SESSION['ks_tables_filters']))
	{
		$tablesFilters = $_SESSION['ks_tables_filters'];
	}
	if(!isset($_POST['ks_tables_filters']))
	{

		$_SESSION["ks_tables_filters"]=$tablesFilters;
	}
	if(is_array($tablesFilters))
	{
		foreach($tablesFilters as $key=>$val)
		{
...
return;
		 ...
		 //check if this relation exists
		  ...
		  {
            var opt= "`"+left_table+"`.`"+left_field+"` <-> "+filters+" "+filter_value+filter_value1;
    ...
    if(opt==optf )
         ...
         addOption = new Option( "`"+left_table+"`.`"+left_field+"` <-> "+filters+" "+filter_value+filter_value1);
	...
	<tr>
		<td align="center" width="64" style="background-repeat: y" valign="top" background="images/leftadd.jpg">
			...
			<table border="0" width="100%" id="table8" height="333">
				<tr>
					<td colspan="2" height="18"><b class="step_title">Tables Filters </b>
					<button id="exit" style="position: relative;left: 375px;font-size: 11px;cursor: pointer;" onclick="return false;">Disconnect &amp; Exit</button>
					</td>
				</tr>
				<tr>
 ...
 echo " <tr><td  height='23' colspan='5' align='center' style='color:red;' valign='top'>The date formate must be YYYY-MM-DD <td> </tr>";
      ...
      ?>
				...
				<tr>
					...
					<td align="center"><a
                  href="<?php  if(count($_SESSION["ks_table"])==1){ echo  "step_3.php";}else{echo  "tables_relations.php";}?>" style="color: #0029a3; text-decoration: none"><img
